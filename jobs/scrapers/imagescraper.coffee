...
module.exports = (cardPath) ->
  ...
  cardService.getAllCards()
    .each (card) ->
      cardPath = setsPath + '/' + card.name + '/' + card.set_name
      ...
      request {url: cardPath, headers: headers}
        .then (response) ->
          ...
          request
            .get(imageurl)
            .pipe(fs.createWriteStream(__dirname + card.name))
            .on('finish', () ->
              imageStream = fs.readFileSync(__dirname + card.name)
              imageBuffer = new Buffer(imageStream, 'binary')
              image = imageBuffer.toString('base64')
              filepicker.store(image, card.name, 'image/jpeg', "")
            )
